...
/**
 * Retrieves the data for a single item from the session.org. The item may be a tune, discussion,
 * recording, session or event.
 * 
 * @author Colman
 * @since 2018-04-01
 */
public class ItemRetriever {
  ...
  /**
   * Gets the details of an individual recording based on its numeric ID in thesession.org. This
   * gets details of the track listing, the tunes on each track, the comments on the recording's
   * page on thesession.org and more.
   * 
   ...
   * gets several items of metadata relating to the discussion, as well as the comments that make up
   * the discussion.
   * 
  ...
  /**
   * Gets the details of an individual tune based on its numeric ID in thesession.org. This gets
   * several items of metadata relating to the tune, as well as any alternative settings, and a list
   * of comments on the tune's page.
   * 
  ...
  /**
   * Gets the details of an individual session based on its numeric ID in thesession.org. This gets
   * several items of metadata relating to the session, as well as the comments on the session's
   * page.
   * 
  ...
  /**
   * Gets the details of an individual event based on its numeric ID in thesession.org. This gets
   * several items of metadata relating to the event, as well as the comments on the event's page.
   * 
   * @return an ItemResultRecording object with the details of the chosen recording
   * @throws IOException if there is a problem with the HTTPS request to the API
   * @throws URISyntaxException if the underlying UrlBuilder class throws a URISyntaxException
  ...
  private ItemResultRecording populateRecordingResult(ItemWrapperRecording parsedResults) {
    ...
    RecordingDetails recordingDetails = new RecordingDetails(parsedResults.id, parsedResults.url,
        StringCleaner.cleanString(parsedResults.name), parsedResults.date);

    // Get the details of the member who originally submitted the recording
    User member = new User(parsedResults.member.id,
        StringCleaner.cleanString(parsedResults.member.name), parsedResults.member.url);

    // Get the details of the recording artist(s)
    Artist artist = new Artist(parsedResults.artist.id,
        StringCleaner.cleanString(parsedResults.artist.name), parsedResults.artist.url);
    ...
    for (int i = 0; i < (parsedResults.tracks.length); i++) {
      ...
      for (int j = 0; j < (parsedResults.tracks[i].tunes.length); j++) {
        TuneDetails currentTune = new TuneDetails(parsedResults.tracks[i].tunes[j].id,
    ...
    // Populate the ArrayList of Comment objects by iterating through each comment in the JSON
    // response
    for (int i = 0; i < (parsedResults.comments.length); i++) {
      // Populate the User object representing the person who submitted the comment
      User commentSubmitter = new User(parsedResults.comments[i].member.id,
          parsedResults.comments[i].member.name, parsedResults.comments[i].member.url);

      // Populate the Comment object with all information related to the comment, including the user
      // set up above
      Comment currentComment =
          new Comment(Integer.parseInt(parsedResults.comments[i].id), parsedResults.comments[i].url,
              StringCleaner.cleanString(parsedResults.comments[i].subject),
              StringCleaner.cleanString(parsedResults.comments[i].content), commentSubmitter,
    ...
    ItemResultRecording finalResult =
        new ItemResultRecording(recordingDetails, member, artist, tracks, comments);
  ...
  private ItemResultDiscussion populateDiscussionResult(ItemWrapperDiscussion parsedResults) {
    ...
    // StringCleaner.cleanString() will decode the &039; etc. XML entities from the JSON response
    DiscussionDetails discussionDetails = new DiscussionDetails(parsedResults.id,
        parsedResults.name, parsedResults.url, parsedResults.date);

    // Get the details of the member who originally submitted the discussion
    User member = new User(parsedResults.member.id,
        StringCleaner.cleanString(parsedResults.member.name), parsedResults.member.url);

    // Initalise an ArrayList of Comment objects to hold each individual comment within the
    // dicussion
    ArrayList<Comment> comments = new ArrayList<Comment>();
    ...
    ItemResultDiscussion finalResult =
        new ItemResultDiscussion(discussionDetails, member, comments);
  ...
  private ItemResultTune populateTuneResult(ItemWrapperTune parsedResults) {
    // Extract each element from the tune entry in the JSON response. StringCleaner.cleanString()
    // will decode the &039; etc. XML entities from the JSON response

    // Get the basic set of tune details
    TuneDetails tuneDetails = new TuneDetails(parsedResults.id,
        StringCleaner.cleanString(parsedResults.name), parsedResults.url);

    // Get the tune type and date of submission
    TuneDetailsWithDate tuneDetailsWithDate =
        new TuneDetailsWithDate(tuneDetails, parsedResults.type, parsedResults.date);
    ...
    // Iterate through each of the alternative titles in the JSON and use them to populate the
    // ArrayList
    ...
    // Populate the ArrayList of TuneSetting objects by iterating through each setting in the JSON
    // response
    for (int i = 0; i < (parsedResults.settings.length); i++) {
      ...
      User settingSubmitter = new User(parsedResults.settings[i].member.id,
          parsedResults.settings[i].member.name, parsedResults.settings[i].member.url);

      // Populate the basic details of the setting
      SettingDetails currentSettingDetails =
      ...
      // Populate the SettingDetailsWithAbc object, adding the abc notation of the tune and the
      // details of the user to the setting details
      SettingDetailsWithAbc currentSetting = new SettingDetailsWithAbc(currentSettingDetails,
          parsedResults.settings[i].abc, settingSubmitter);
    ...
    ItemResultTune finalResult = new ItemResultTune(tuneDetailsWithDate, member, tunebooks,
        recordings, aliases, settings, comments);
  ...
  private ItemResultSession populateSessionResult(ItemWrapperSession parsedResults) {
    ...
    SessionDetails sessionDetails =
        new SessionDetails(parsedResults.id, parsedResults.url, parsedResults.date);
    Coordinates coordinates = new Coordinates(parsedResults.latitude, parsedResults.longitude);
    User member = new User(parsedResults.member.id,
        StringCleaner.cleanString(parsedResults.member.name), parsedResults.member.url);
    Venue venue =
        new Venue(parsedResults.venue.id, StringCleaner.cleanString(parsedResults.venue.name),
            parsedResults.venue.telephone, parsedResults.venue.email, parsedResults.venue.web);
    ...
    // Initialize an ArrayList of Strings to store the schedule, i.e. list of days when the session
    // happens
    ...
    // Instantiate a DiscussionByIDResult object & populate it with the details captured above
    ItemResultSession finalResult = new ItemResultSession(sessionDetails, coordinates, member,
        venue, town, area, country, schedule, comments);
  ...
  private ItemResultEvent populateEventResult(ItemWrapperEvent parsedResults) {
    ...
    EventDetails eventDetails = new EventDetails(parsedResults.id,
        StringCleaner.cleanString(parsedResults.name), parsedResults.url, parsedResults.date);
    User member = new User(parsedResults.member.id,
        StringCleaner.cleanString(parsedResults.member.name), parsedResults.member.url);
    ...
    // Instantiate a DiscussionByIDResult object & populate it with the details captured above
    ItemResultEvent finalResult = new ItemResultEvent(eventDetails, member, schedule, coordinates,
        venue, town, area, country, comments);
