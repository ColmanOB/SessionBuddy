...
/**
 * Retrieves a list of most-recently added entries in a chosen category - tunes, discussions,
 * recordings, events or sessions.
 * 
 * @author Colman O'B
 * @since 2018-04-01
 */
public class LatestSearch extends Search {
  ...
  public ArrayList<SearchResultTunesLatest> listTunes()
    ...
    try {
      ...
      // Create an ArrayList of SearchResultTunesLatest objects to hold the parsed response from the
      // API, and populate it with a helper method
  ...
  public ArrayList<SearchResultDiscussions> listDiscussions()
    ...
    try {
      ...
      // Create an ArrayList of SearchResultDiscussions objects to hold the parsed response from the
      // API, and populate it with a helper method
  ...
  /**
   * Retrieves a list of recently added recordings on thesession.org, with the most recent results
   * first
   * 
  ...
  public ArrayList<SearchResultRecordings> listRecordings()
    ...
    try {
      ...
      // Create an ArrayList of SearchResultRecordings to hold the parsed response from the API, and
      // populate it with a helper method
  ...
  /**
   * Retrieves a list of recently added sessions on thesession.org, with the most recent results
   * first
   * 
  ...
  public ArrayList<SearchResultSessions> listSessions()
    ...
    try {
      ...
      // Create an ArrayList of SearchResultSessions to hold the parsed response from the API, and
      // populate it with a helper method
  ...
  public ArrayList<SearchResultEvents> listEvents()
    ...
    try {
      ...
      // Create an ArrayList of SearchResultEvents objects to hold the parsed response from the API,
      // and populate it with a helper method
   ...
   * 
   * @param parsedResults an existing KeywordSearchWrapperRecordings object that has already been
   *        populated
   * @return an ArrayList of SearchResultRecordings objects
   * 
   * @author Colman
  ...
  private URL composeURL(String dataCategory) throws MalformedURLException, URISyntaxException {
    ...
    if (pageNumber > 0) {
      ...
      requestURL = builder.new Builder().requestType(RequestType.SEARCH_LATEST_ITEMS)
          .path(dataCategory + "/" + "new").itemsPerPage(resultsPerPage).pageNumber(pageNumber)
          .build();
    }
    ...
    // If no page is specified
      ...
      requestURL = builder.new Builder().requestType(RequestType.SEARCH_LATEST_ITEMS)
          .path(dataCategory + "/" + "new").itemsPerPage(resultsPerPage).build();
    }
