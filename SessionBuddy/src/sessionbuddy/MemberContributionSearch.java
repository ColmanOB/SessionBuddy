...
/**
 * Retrieves a list of member contributions in a chosen category - tunes, discussions, recordings,
 * events, sessions or sets.
 * 
 * @author Colman O'B
 * @since 2018-03-11
 */
public class MemberContributionSearch extends Search {
   ...
   * @since 2018-04-01
   */
  public ArrayList<SearchResultTunesLatest> listTunes()
    ...
    try {
      ...
      LatestWrapperTunes parsedResults =
          JsonParser.parseResponse(response, LatestWrapperTunes.class);

      // Create an ArrayList of SearchResultTunesLatest objects to hold each individual search
      // result entry, and return the ArrayList
  ...
  /**
   * Retrieves a list of recordings added by a particular user on thesession.org, with the most
  ...
  public ArrayList<SearchResultRecordings> listRecordings()
    ...
    try {
      ...
      // Parse the returned JSON into a wrapper class to allow access to all elements
      KeywordSearchWrapperRecordings parsedResults =
          JsonParser.parseResponse(response, KeywordSearchWrapperRecordings.class);

      // Create an ArrayList of SearchResultRecordings objects to hold each individual search result
      // entry, and return the ArrayList
  ...
  /**
   * Retrieves a list of sessions added by a particular user on thesession.org, with the most recent
   * results first
   * 
   ...
   * @since 2018-04-01
   */
  public ArrayList<SearchResultSessions> listSessions()
      throws IllegalArgumentException, IOException, URISyntaxException {
    try {
      // Validate that a number between 1-50 has been provided as the resultsPerPage value
      validateResultsPerPageCount(resultsPerPage);
      ...
      // Parse the returned JSON into a wrapper class to allow access to all elements
      KeywordSearchWrapperSessions parsedResults =
          JsonParser.parseResponse(response, KeywordSearchWrapperSessions.class);

      // Create an ArrayList of SearchResultSessions objects to hold each individual search result
      // entry, and return the ArrayList
   ...
   * @since 2018-04-01
   */
  public ArrayList<SearchResultEvents> listEvents()
      throws IllegalArgumentException, IOException, URISyntaxException {
    try {
      // Validate that a number between 1-50 has been provided as the resultsPerPage value
      validateResultsPerPageCount(resultsPerPage);
      ...
      // Parse the returned JSON into a wrapper class to allow access to all elements
      KeywordSearchWrapperEvents parsedResults =
          JsonParser.parseResponse(response, KeywordSearchWrapperEvents.class);

      // Create an ArrayList of SearchResultEvents objects to hold each individual search result
      // entry, and return the ArrayList
   ...
   * @since 2018-04-01
   */
  public ArrayList<SearchResultDiscussions> listDiscussions()
      throws IllegalArgumentException, IOException, URISyntaxException {
    try {
      // Validate that a number between 1-50 has been provided as the resultsPerPage value
      validateResultsPerPageCount(resultsPerPage);
      ...
      // Instantiate a DiscussionSearchParser and DiscussionSearchResultWrapper needed to handle the
      // raw JSON
      KeywordSearchWrapperDiscussions parsedResults =
          JsonParser.parseResponse(response, KeywordSearchWrapperDiscussions.class);

      // Create an ArrayList of SearchResultDiscussion objects to hold each individual search result
      // entry, and return the ArrayList
  ...
  public ArrayList<SearchResultSets> listSets()
    ...
    try {
      ...
      // Build the URL using a helper method and pass it in to the HttpRequstor.submitRequest(URL)
   ...
   * @since 2018-02-10
   */
  private ArrayList<SearchResultTunesLatest> populateTunesSearchResult(
      LatestWrapperTunes parsedResults) {
    ...
    for (int i = 0; i < parsedResults.settings.length; i++) {
      ...
      SettingDetails details =
          new SettingDetails(parsedResults.settings[i].id, parsedResults.settings[i].url,
              parsedResults.settings[i].key, parsedResults.settings[i].date);
      ...
      TuneDetails settingDetails = new TuneDetails(parsedResults.settings[i].tune.id,
          parsedResults.settings[i].tune.name, parsedResults.settings[i].tune.url);

      // Instantiate a TunesSearchResult object & populate it
      SearchResultTunesLatest currentResult =
          new SearchResultTunesLatest(details, submitter, settingDetails);
  ...
  /**
   * Helper method to gather and parse the response to a search for recordings that have been added
   ...
   * @since 2018-02-10
   */
  private ArrayList<SearchResultRecordings> populateRecordingsSearchResult(
      KeywordSearchWrapperRecordings parsedResults) {
    ...
    // Find out how many pages are in the response, to facilitate looping through multiple pages if
    // needed
    pageCount = Integer.parseInt(parsedResults.pages);
    ...
    for (int i = 0; i < (parsedResults.recordings.length); i++) {
      // Extract the required elements from each individual search result in the JSON response
      // StringCleaner.cleanString() will decode the &039; etc. XML entities from the JSON response
      RecordingDetails details = new RecordingDetails(parsedResults.recordings[i].id,
          StringCleaner.cleanString(parsedResults.recordings[i].name),
          parsedResults.recordings[i].url, parsedResults.recordings[i].date);
  ...
  /**
   * Helper method to gather and parse the response to a search for sessions added by a particular
   * user
   * 
   * @param parsedResults an existing KeywordSearchWrapperSessions object that has been populated
   * @return an ArrayList of SearchResultSessions objects
   ...
   * @since 2018-02-18
   */
  private ArrayList<SearchResultSessions> populateSessionsSearchResult(
      KeywordSearchWrapperSessions parsedResults) {
    // Use a TunesSearchParser to parse the raw JSON into a usable structure using Gson
    ...
    for (int i = 0; i < (parsedResults.sessions.length); i++) {
      // Extract the required elements from each individual search result in the JSON response
      // StringCleaner.cleanString() will decode the &039; etc. XML entities from the JSON response
      SessionDetails details = new SessionDetails(parsedResults.sessions[i].id,
          parsedResults.sessions[i].url, parsedResults.sessions[i].date);
      Coordinates coordinates =
          new Coordinates(parsedResults.sessions[i].latitude, parsedResults.sessions[i].longitude);
      ...
      Venue venue = new Venue(parsedResults.sessions[i].venue.id,
          ...
          parsedResults.sessions[i].venue.telephone, parsedResults.sessions[i].venue.email,
      ...
      SearchResultSessions currentResult =
          new SearchResultSessions(details, coordinates, submitter, venue, town, area, country);
  ...
  /**
   * Helper method to gather and parse the response to a search for events added by a particular
   * user
   * 
   ...
   * @since 2018-02-18
   */
  private ArrayList<SearchResultEvents> populateEventsSearchResult(
      KeywordSearchWrapperEvents parsedResults) {
    // Use a TunesSearchParser to parse the raw JSON into a usable structure using Gson
    ...
    for (int i = 0; i < (parsedResults.events.length); i++) {
      ...
      EventDetails details = new EventDetails(parsedResults.events[i].id,
          StringCleaner.cleanString(parsedResults.events[i].name), parsedResults.events[i].url,
          parsedResults.events[i].date);
      Coordinates coordinates =
          new Coordinates(parsedResults.events[i].latitude, parsedResults.events[i].longitude);
      EventSchedule schedule =
          new EventSchedule(parsedResults.events[i].dtstart, parsedResults.events[i].dtend);
      ...
      Venue venue = new Venue(parsedResults.events[i].venue.id,
          ...
          parsedResults.events[i].venue.telephone, parsedResults.events[i].venue.email,
      ...
      SearchResultEvents currentResult = new SearchResultEvents(details, submitter, schedule,
          coordinates, venue, town, area, country);
  ...
  /**
   * Helper method to gather and parse the response to a search for discussions submitted by a
   * particular user
   * 
   ...
   * @since 2018-02-23
   */
  private ArrayList<SearchResultDiscussions> populateDiscussionsSearchResult(
      KeywordSearchWrapperDiscussions parsedResults) {
    // Use a TunesSearchParser to parse the raw JSON into a usable structure using Gson
    ...
    for (int i = 0; i < (parsedResults.discussions.length); i++) {
      ...
      // StringCleaner.cleanString() will decode the &039; etc. XML entities from the JSON response
      DiscussionDetails details = new DiscussionDetails(parsedResults.discussions[i].id,
          StringCleaner.cleanString(parsedResults.discussions[i].name),
          parsedResults.discussions[i].url, parsedResults.discussions[i].date);
  ...
  private ArrayList<SearchResultSets> populateSetSearchResult(LatestWrapperSets parsedResults) {
    ...
    for (int i = 0; i < parsedResults.sets.length; i++) {
      // Extract the required elements from each individual search result in the JSON response
      // StringCleaner.cleanString() will decode the &039; etc. XML entities from the JSON response
      SetDetails details = new SetDetails(parsedResults.sets[i].id,
          StringCleaner.cleanString(parsedResults.sets[i].name), parsedResults.sets[i].url,
          parsedResults.sets[i].date);
      User submitter = new User(parsedResults.sets[i].member.id,
  ...
  private URL composeURL(String dataCategory) throws MalformedURLException, URISyntaxException {
    ...
    if (pageNumber > 0) {
      ...
      requestURL = builder.new Builder().requestType(RequestType.SEARCH_MEMBER_CONTRIBUTIONS)
          .path("members" + "/" + userID + "/" + dataCategory).itemsPerPage(resultsPerPage)
    ...
    // If no page is specified
      ...
      requestURL = builder.new Builder().requestType(RequestType.SEARCH_MEMBER_CONTRIBUTIONS)
          .path("members" + "/" + userID + "/" + dataCategory).itemsPerPage(resultsPerPage).build();
    }
