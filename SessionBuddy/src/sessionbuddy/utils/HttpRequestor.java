...
/**
 * Submits a HTTPS GET request to the API at https://thesession.org, using a URL in the format
 * required by the API. The JSON response is captured as a String and returned to the caller. A
 * standard user of this library should never have to instantiate an object of this class directly -
...
public class HttpRequestor {
  // The API can return data as JSON, RSS or XML - we are only using XML in this project
  private static final String dataFormat = "json";


  /**
   * Submits a request to thesession.org API, where the URL passed in as an argument is the URL
   * pointing to the desired API resource
   * 
   * @param requestURL A URL pointing to specific resource on thesession.org as per
   ...
   * 
   * @param connectionToURL a HttpURLConnection to API, built with a URL in the format required by
   *        the API
   * @return a String containing either the entire JSON response from the API, or a specific page of
   *         the response
   * @throws IOException if there is a problem setting up the connection or reading data from it
   */
  private static String captureResponse(HttpURLConnection connectionToURL) throws IOException {
    try {
      ...
      BufferedReader inputReader =
          new BufferedReader(new InputStreamReader((connectionToURL.getInputStream()), "utf-8"));
  ...
  /**
   * Makes a HTTP connection to the API with the requested data in the URL, and get the response
   * data
   * 
   ...
   * @throws IOException if there was a problem establishing the connection to the API or reading a
   *         response
   */
