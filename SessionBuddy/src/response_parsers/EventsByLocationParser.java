...
/**
 * Uses GSON to parse a set of event search results from thesession.org API
 * 
 * @author Colman O'B
 * @since 2017-08-27
 */
public class EventsByLocationParser 
	{
	// Instantiate a wrapper object with fields that exactly match those in the search results JSON string
	private EventsByLocationWrapper listOfResults;	
	 ...
	 */
	public EventsByLocationWrapper parseResponse(String searchResultsString) 
		{
		...
		// Populate the Gson object using the string of JSON returned from the API
			{
			listOfResults = gson.fromJson(searchResultsString, EventsByLocationWrapper.class);
			}
		...
		// Return the EventsByLocationWrapper object
